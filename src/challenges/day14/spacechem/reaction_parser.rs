...
fn parse_reaction(s: &str) -> Result<Reaction> {
    ...
    let (inputs, outputs) = parts
        .next()
        .and_then(|q| parts.next().map(|m| (q, m)))
...
fn parse_quantity(s: &str) -> Result<Quantity> {
    let mut parts = s.split(' ').filter(|s| !s.is_empty());
    let (q_str, m_str) = parts
        .next()
        .and_then(|q| parts.next().map(|m| (q, m)))
        .ok_or_else(|| ah!("could not parse quantity from: {}", s))?;
    let quantity =
        parse_str::<i64>(q_str).map_err(|e| ah!("could not parse quantity from: {}: {}", s, e))?;
    Ok(Quantity {
        amount: quantity,
...
mod tests {
    ...
    fn parse_input_list_single() {
        assert_eq!(
            parse_input_list("32 azlp").unwrap(),
            vec![Quantity {
                amount: 32,
                molecule: Molecule::from("azlp")
            }]
    ...
    fn parse_input_list_long() {
        assert_eq!(
            parse_input_list("53 STKFG, 6 MNCFX, 46 VJHF, 81 HVMC, 68 CXFTF, 25 GNMV").unwrap(),
            vec![
                Quantity {
                    amount: 53,
                    molecule: "STKFG".into()
    ...
    fn parse_compact_reaction() {
        assert_eq!(
            parse_reaction("53 A,6 B,=>1 C").unwrap(),
            Reaction {
                inputs: vec![
                    Quantity {
                        amount: 53,
    ...
    fn parse_reaction_complex() {
        assert_eq!(
            parse_reaction("53 STKFG, 6 MNCFX, 46 VJHF, 81 HVMC, 68 CXFTF, 25 GNMV => 1 FUEL")
                .unwrap(),
            Reaction {
                inputs: vec![
                    Quantity {
    ...
    fn parse_manifest() {
        assert_eq!(
            parse_reaction_manifest("53 A,6 B,=>1 C\n4 C, 1 A => 1 CA").unwrap(),
            vec![
                Reaction {
                    inputs: vec![
                        Quantity {
